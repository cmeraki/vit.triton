...
def matmul_kernel(
    ...
    input_row_mask = input_row_offset < num_rows
    input_row_offset = input_row_offset[:, None] * input_row_stride

    input_col_offset = tl.arange(0, num_input_cols)
    input_col_mask = input_col_offset < num_input_cols
    input_col_offset = input_col_offset[None, :]

    input_data_ptr = input_ptr + input_batch_offset + input_row_offset + input_col_offset
    input_data = tl.load(input_data_ptr, mask=(input_row_mask[:, None] and input_col_mask[None, :]))

    # Offsets for weight data
    weight_row_offset = tl.arange(0, num_input_cols)
    weight_row_mask = weight_row_offset < num_input_cols
    weight_row_offset = weight_row_offset[:, None] * weight_col_stride

    weight_col_offset = col_idx*BLOCK_SIZE_COL + tl.arange(0, BLOCK_SIZE_COL)
    weight_col_mask = weight_col_offset < num_output_cols
    weight_col_offset = weight_col_offset[None, :] * weight_col_offset

    weight_data_ptr = weight_ptr + weight_row_offset + weight_col_offset
    weight_data = tl.load(weight_data_ptr, mask=(weight_row_mask[:, None] and weight_col_mask[None, :]))
    ...
    output_row_mask = output_row_offset < num_rows
    ...
    output_col_mask = output_col_offset < num_output_cols
    output_col_offset = output_col_offset[None, :]

    output_data_ptr = output_ptr + output_batch_offset + output_row_offset + output_col_offset
    tl.store(output_data_ptr, result, mask=(output_row_mask[:, None] and output_col_mask[None, :]))
